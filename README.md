...
```mermaid
flowchart TD
    A([👤 Customer]) -->|upload / mail-in| B[🌐 AprilDawn Web App<br/>Next.js + React]
    B --> C[🔌 API Routes<br/>upload · quote · reminders · contact]
    C --> D[(🗄️ Object Storage<br/>S3 / Cloudflare R2)]
    C --> E[(🧠 Postgres<br/>orders · vault · reminders)]
    C --> F[🪄 Media Pipeline<br/>scan · restore · transcribe]
    F --> G[🤖 AI Services<br/>Claude · restoration models]
    C --> H[🖨️ Print Fulfillment<br/>Printful / Printify / makers]
    C --> I[💳 Stripe Payments]
    C --> J[📅 Integrations<br/>Google · Outlook · Apple · Facebook]
    C --> K[📨 Notifications<br/>email · SMS · push]
    H -->|select items| L[🛒 Amazon Associates]
```
